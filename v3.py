...
login_page_url = 'https://pass.sdu.edu.cn/cas/login?service=https%3A%2F%2Fscenter.sdu.edu.cn%2Ftp_fp%2Findex.jsp'
order_page_url = 'https://scenter.sdu.edu.cn/tp_fp/#from=hall&serveID=755b2443-dda6-47b6-ba0c-13f5f1e39574&act=fp/serveapply'
check_url = 'https://scenter.sdu.edu.cn/tp_fp/fp/serveapply/checkService'
logout_url = 'https://scenter.sdu.edu.cn/tp_fp/logout'
form_url = 'https://scenter.sdu.edu.cn/tp_fp/formParser?status=codeList'
...
class AutoOrder:
    ...
    #         return False
    def jump_to_order_page(self):
        """
        先判断是否开放约场了，开放了就跳转过去
        :return: res --> boolean
        """
        res = self.session.get(order_page_url, allow_redirects=False)
        while res.status_code == 302:
            res = self.session.get(order_page_url, allow_redirects=False)
            print('开放还没开放了')
            time.sleep(5)
        print('开放约场了' + str(datetime.datetime.now()))
        self.jump(order_page_url) # 跳转到约场页面

    def get_field_info(self):
        filed_data = {'codelist_type': 'pbrq_fyccbh_sj_qd_4', 'formid': '408225d8-1abe-4cdd-8a0d-fd8b1c6f', 'codelist_parentValue': '2022-03-23青岛校区'}
        place_res = self.session.post(url=form_url, data=filed_data).text
        print(place_res)
        json.loads(place_res)    # 将json字符串转换成字典
